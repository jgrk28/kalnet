...
@dataclass(frozen=True)
class GainCorrelationResult:
    """Pooled summary stats and their Pearson correlations with the input gain g.

    This replicates Orhan & Ma (2017) Fig. 6a/b: correlation of (a) mean
    hidden-layer activity and (b) hidden-layer kurtosis (sparsity) with the
    mean input response, for which trial gain g is used here as the driving
    variable (mean Poisson input response scales with g).
    """
...
def correlate_hidden_vs_gain(
    r_hid: torch.Tensor | np.ndarray,
    gain: torch.Tensor | np.ndarray,
) -> GainCorrelationResult:
    """Correlate Mean activity and Kurtosis with input gain g (paper's Fig. 6a/b).

    Note: this is the paper's original analysis (hidden stats vs. mean input
    response / gain), distinct from `correlate_hidden_stats`, which relates
    hidden stats to the *true* posterior variance sigma_sq -- a quantity the
    original paper did not have in closed form for most tasks and therefore
    did not test directly.
    """
    # pool_hidden_and_opt_variance is a generic (r_hid, scalar-per-sample)
    # pooling routine despite its name; reused here for the gain variable.
    r_hid_flat, g = pool_hidden_and_opt_variance(r_hid, gain)
    mean_activity, kurt = hidden_layer_stats(r_hid_flat)
    r_mean, p_mean = pearsonr(mean_activity, g)
    r_kurt, p_kurt = pearsonr(kurt, g)
    return GainCorrelationResult(
        mean_activity=mean_activity,
        kurtosis=kurt,
        gain=g,
        r_mean=float(r_mean),
